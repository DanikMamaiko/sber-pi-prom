...
`items/from-jira` требует `backlog:write`, получает `issue_key`, `tribe`, резервную
команду доски `fallback_team` и `expected_version`. Backend обращается к Jira SW от
имени сервисной учётной записи, переводит секунды в чел/дни, сопоставляет команды с
активным PI-циклом и атомарно создаёт элемент бэклога. Если Jira отключена, недоступна,
не ответила за отведённое время или вернула `404`, создаётся пустая инициатива с
введённым Issue, выбранными трайбом и командой; ответ содержит `jira: null` и
предупреждение о ручном заполнении. Ошибки доступа и некорректные ответы Jira не
запускают резервный сценарий. При успешном импорте ответ содержит новый `board`,
нормализованный снимок `jira` и предупреждения маппинга. Исходный Jira-снимок также
сохраняется в `backlog_items.jira_issue_data`; секреты Jira в ответ не включаются.

Маппинг оценок в компетенции: проектирование и анализ — `SA`, дизайн — `DES`,
разработка и другое — `DEV`, тестирование и ПСИ — `QA`. Если компетенция не включена
у выбранной команды PI-цикла, импорт продолжается, но соответствующая оценка остаётся
только в Jira-снимке и возвращается предупреждение.

Исходящие запросы к Jira ограничены на уровне процесса backend: одновременно выполняется
не более `JIRA_MAX_CONCURRENT_REQUESTS` запросов, а очередь содержит не более
`JIRA_MAX_QUEUE_SIZE` ожидающих запросов. Если очередь заполнена или запрос не получил
