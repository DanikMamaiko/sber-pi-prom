...
- Для PM (`business_viewer`) вкладка «Бэклог команд» доступна только на чтение
  (`backlog:read`), запись бэклога остаётся запрещённой (`403`). PRE PI доступна PM
  и на чтение, и на изменение (`pre_pi:write`).
...
- Вкладка `backlog` доступна `business_viewer` только на чтение (read-only).
...
- `business_viewer` читает бэклог (read-only), но получает `403` на его изменение;
...
- `business_viewer` видит backlog только read-only, не видит и не загружает PI data;
