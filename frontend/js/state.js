...
// DEV_TYPE — тип «Развитие функционала»: для него на бэкенде становятся обязательными
// поля цели. Канонические типы Initiative Types сравниваются с бэкендом по точному
// совпадению строки, поэтому в UI их выбирают из списка (см. initiativeTypeFieldHTML).
const DEV_TYPE='Развитие функционала';
// REG_TYPE — тип «Требования законодательства»: учитывается отдельным блоком «Регуляторка»
// на вкладке Pre PI Planning. LEGAL_OWNER_TEAM — команда-владелец, для которой регуляторка
// считается как «общая» (для остальных владельцев — «командная»); сравнение точное.
const REG_TYPE='Требования законодательства';
const LEGAL_OWNER_TEAM='Legal';
const INITIATIVE_TYPES=[DEV_TYPE,TECH_TYPE_COMMON,TECH_TYPE_TEAM,REG_TYPE];
// Сентинель опции «Другое…» в поле типа: не сохраняется, только раскрывает свободный ввод.
const INITIATIVE_TYPE_OTHER='__other__';
const PI_TABS = [
